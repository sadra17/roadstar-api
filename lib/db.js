...
const Bookings = {
  ...
  async find(filter = {}, options = {}) {
    ...
    // orSearch: array of [column, value] pairs joined with Supabase .or()
    // e.g. [["first_name","john"],["phone","416"]] → first_name.ilike.%john%,phone.ilike.%416%
    if (options.orSearch?.length) {
      const clause = options.orSearch.map(([col, val]) => `${col}.ilike.%${val}%`).join(",");
      q = q.or(clause);
    }
    if (options.orderBy) {
...
const SmsLog = {
  ...
  // Batch load SMS logs for multiple bookings.
  // Chunk the id list: a single .in() with hundreds of UUIDs builds a URL that
  // exceeds Supabase's request-line length limit and fails with a 500. Split
  // into batches of 100 ids and merge the results.
  async findByBookingBatch(bookingIds) {
    ...
    for (let i = 0; i < bookingIds.length; i += CHUNK) {
      ...
      const { data, error } = await sb.from("sms_log")
        ...
        .in("booking_id", slice)
        .order("sent_at", { ascending: false });
      check(error, "SmsLog.findByBookingBatch");
      if (data) rows.push(...data);
    }
    const map = {};
    for (const row of rows) {
      const r = toCamel(row);
      if (!map[r.bookingId]) map[r.bookingId] = [];
      map[r.bookingId].push(r);
    }
    return map;
  },
...
const Users = {
  ...
  // Permanently removes the row — email is freed up for re-use immediately
  async hardDelete(id) {
    const { error } = await sb.from("users").delete().eq("id", id);
    check(error, "Users.hardDelete");
  },
};
...
const Shops = {
  ...
  // Hard delete — only used for EH7 rollback on failed shop creation
  async delete(shopId) {
    const { error } = await sb.from("shops").delete().eq("shop_id", shopId);
    check(error, "Shops.delete");
  },
};
